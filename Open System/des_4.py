...
def packet_generator(env, number, out_pipe):
    for i in range(number):
        ...
        d = {1: arrival_time, 2: processing_time_1, 3: processing_time_2, 4: processing_time_3, 5: processing_time_4}
        out_pipe.put(d)
...
def server_3(env, in_pipe, out_pipe):
    while True:
        request = yield in_pipe.get()
...
pipe_4 = simpy.Store(environment)
...
environment.process(server_3(environment,pipe_3, pipe_4))
environment.process(server_4(environment,pipe_4))
environment.run()
